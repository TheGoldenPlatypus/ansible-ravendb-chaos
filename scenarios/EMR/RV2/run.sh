#!/usr/bin/env bash
# scenarios/EMR/RV2/run.sh -- teardown + T4 lab (sharded src @ v_old + non-sharded tgt @ v_new) + rv2.yml.
...
step "rv2";             ansible-playbook scenarios/EMR/RV2/rv2.yml "${C[@]}" -e v_old="$V_OLD" -e v_new_build="$V_NEW" "${EXTRA[@]}"
