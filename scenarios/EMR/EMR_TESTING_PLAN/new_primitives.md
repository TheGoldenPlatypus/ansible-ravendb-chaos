# EMR Testing Plan — Proposed Toolbox Primitives
...
**Helper.** `EMR_TESTING_PLAN/tools/sink_projection.py` — pure Python; consumes workload-trace JSONL + filter-mutation JSONL; outputs the count (and on forensic, the set).
