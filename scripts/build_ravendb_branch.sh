#!/usr/bin/env bash
###################################################################################################
# scripts/build_ravendb_branch.sh
#
# Build a RavenDB .deb (Studio included) from a ravendb/ravendb BRANCH (not a PR).
# Outputs <repo_root>/builds/raven-branch-<flattened-name>.deb that you hand to
# install_ravendb.yml's `custom_build` param.
#
# Sibling of build_ravendb_pr.sh -- the only difference is the fetch/checkout block.
#
# USAGE:
#   scripts/build_ravendb_branch.sh <branch-name>
#
# EXAMPLE:
#   scripts/build_ravendb_branch.sh feature/revisions
#   # -> builds/raven-branch-feature-revisions.deb
#   ansible-playbook playbooks/install_ravendb.yml -K \
#       -e custom_build=$PWD/builds/raven-branch-feature-revisions.deb --skip-tags download
#
# WHAT IT DOES (identical to build_ravendb_pr.sh):
...
if [ -z "$BRANCH" ]; then
  ...
  echo "Example: $0 feature/revisions" >&2
  exit 2
fi

# Flatten slashes for the local branch name + output filename: feature/revisions -> feature-revisions
BRANCH_FLAT="${BRANCH//\//-}"
...
FINAL="${BUILDS_DIR}/raven-branch-${BRANCH_FLAT}.deb"
...
# when a previous run left this branch checked out.
git fetch origin "$BRANCH"
git checkout -B "branch-${BRANCH_FLAT}" FETCH_HEAD
git submodule update --init --recursive 2>/dev/null || true
HEAD_SHA=$(git rev-parse --short HEAD)
green "==> branch '${BRANCH}' checked out at $HEAD_SHA"
echo
...
cp "$DEB" "$FINAL"
echo
green "==> DONE  ${FINAL}  (branch '${BRANCH}', ${HEAD_SHA}, ${RAVENDB_VERSION})"
echo
echo "Use it:"
